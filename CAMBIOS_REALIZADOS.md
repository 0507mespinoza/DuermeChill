...
- El Login requería registro manual y contraseñas (agregado Login con Google)
- El Modo Oscuro no se aplicaba correctamente a todas las pantallas, dejando colores fijos ilegibles
...
**Solución:** - Modificar la consulta para retornar todos los registros del reloj de los últimos 30 días
...
Actualización de la estructura de datos del reloj:
...
Creación de una nueva vista dedicada a la edición del perfil.

Agregó control de errores mediante validación de campos para evitar nombres vacíos.

Se integró un botón de eliminación de cuenta con AlertDialog preventivo.

Actualización de PantallaYo para añadir un botón de "Editar mis datos" con refresco automático de la interfaz al volver.
...
Se aplicó "Inyección de Contexto" en el método _enviarPregunta().

Ahora se extrae la información de la caché de la última noche del usuario (horas, calidad, despertares) y se adjunta de forma invisible al prompt.
...
7. Integración de Login con Google (NUEVO)
Backend y Frontend

Configuración de Firebase y Google Cloud (API Key, App ID, OAuth 2.0 y People API).

Integración del paquete google_sign_in en Flutter para soporte Web y Android.

Creación del endpoint /auth/google en Spring Boot para validar el token y registrar/loguear al usuario automáticamente sin pedir contraseña.

8. Modo Oscuro Global (NUEVO)
UI/UX en todas las pantallas

Implementación de un SwitchListTile interactivo en mi_drawer.dart para cambiar el tema manualmente y en tiempo real.

Adaptación reactiva (isDark ? colorOscuro : colorClaro) en PantallaPrincipal, PantallaEstadisticas, PantallaEncuesta, PantallaCoach y PantallaAlarmas.

Eliminación de colores "quemados" o fijos que rompían la legibilidad durante la noche (AppBars, fondos de tarjetas, textos e iconos adaptados).
...
flutter clean
flutter pub get
# Para probar el Login en Web, arrancar forzando el puerto autorizado:
flutter run -d chrome --web-port 5000
Paso 3: Probar el Flujo

Inicia sesión con tu usuario (o regístrate desde cero, o usa el botón de Google).
...
Paso 4: Probar Nuevas Funciones (Perfil, IA, Tema y Autenticación)
...
Login: Cierra sesión y entra usando el botón de Google.

Tema Oscuro: Abre el menú lateral y usa el interruptor del Modo Oscuro. Navega por las pantallas para comprobar la adaptación visual.
...
Acceso Inmediato: Login en un clic con Google sin recordar contraseñas
 Cuidado Visual: Modo Oscuro 100% integrado para proteger la vista en entornos nocturnos
